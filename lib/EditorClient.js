...
class EditorClient extends Client {
  constructor (data, serverAdapter, editorAdapter) {
    ...
    this.editorAdapter = editorAdapter  ////RichTextCodeMirrorAdapter
  ...
  onChange (textOperation, inverse) {
    ...
    //父类Client的方法applyClient，把操作发给服务器端，在父类Client转来转去最后是调用的是sendOperation（发送操作给服务器）方法，
    //这个方法需要子类即这个类EditorClient重写
    //EditorClient同时也重写了applyOperation方法用于应用服务器发给其他客户端的操作
