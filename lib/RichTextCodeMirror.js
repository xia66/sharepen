...
class RichTextCodeMirror {
  ...
  setAttribute (attribute, value) {
    ...
    //是否选中了文本，没选中文本则直接设置currentAttributes_，
    ...
    } else {  //选中文本则把选中的开始和结束位置以及一个函数传给updateTextAttributes函数
  ...
  updateTextAttributes (start, end, updateFn, origin, doLineAttributes) {
    ...
    this.annotationList_.updateSpan(new Span(start, end - start), function (annotation, length) {
      ...
      // changedAttributes是新值
      // changedAttributesInverse是旧值
  ...
  replaceText (start, end, text, attributes, origin) {
    // outstandingChanges_: apply the edit operations from server, store the rich-text attr in the outstandingChanges_ obj
    // first insert the plain text, then in the change callback ,use the attr to style the text
    //
  ...
  format () {
    ...
    this._mouseupHandler = () => { this._applyFormatBrushIfNeed() }
    ...
    if (!Utils.emptyAttributes(this.formatBrushAttrs_.spanAttrs)) {
      if (this.emptySelection_()) { // cursor
        ...
        this.updateTextAttributes(
          ...
          (attrs) => { Object.assign(attrs, this.formatBrushAttrs_.spanAttrs) }
        )
      }
  ...
  onCodeMirrorChange_ (cm, cmChanges) {
    //console.log(cm.getValue())  //这里就可以获得文本的内容，但是只是一个单纯的字符串，没有样式
